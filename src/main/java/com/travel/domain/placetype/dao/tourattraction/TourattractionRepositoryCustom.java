package com.travel.domain.placetype.dao.tourattraction;

import com.travel.domain.placetype.entity.ApiTag;
import com.travel.domain.placetype.entity.tourattraction.SubjectiveTag;
import com.travel.domain.placetype.entity.tourattraction.TourattractionTag;

import java.util.List;

public interface TourattractionRepositoryCustom {
    List<String> findPlaceGoogleIdsByAddressAndTourattractionTags(String address, TourattractionTag tourattractionTag);
    List<String> findPlaceGoogleIdsByAddressAndSubjectiveTags(String address, SubjectiveTag subjectiveTag);
}
